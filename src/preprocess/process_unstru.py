...
model_path = '/home/ziyihu/.cache/modelscope/hub/ZJUNLP/OneKE'
...
language = 'zh'
schema ={
    'disease_has_drug':'该关系类型描述的是疾病和药物的关系。展示了药物所治疗的疾病。主体是药物，客体是疾病。',
    'disease_has_treatment':'该关系类型描述的是疾病和治理方法的关系。展示了治疗疾病所采用的治疗方法。主体是疾病，客体是治疗方法。',
    'disease_no_eat':'该关系类型描述的是疾病和食物的关系。展示了疾病患病期间不能吃的食物。主体是疾病，客体是食物。',
    'disease_do_check':'该关系类型描述的是疾病和检查的关系。展示了疾病所需要的检查方式。主体是疾病，客体是检查项目。',
    'disease_belong_to_department':'这个关系类型用来表示疾病，科室的关系。展示了疾病所属科室。主体是疾病，客体是科室名称',
    'disease_has_symptom':'该关系类型描述的是疾病和症状的关系。展示了疾病患病时所展现的症状。主体是疾病，客体是症状。',
    'disease_easy_get_population':'该关系类型描述的是疾病和不同群体的关系，展现了疾病的易患病人群。主体是疾病，客体是不同群体名称，如老人，男性，女性，青少年等。',
    'disease_has_prevent':'该关系描述的是疾病和预防措施的关系，展现了预防疾病所需采取的措施。主体是疾病，客体是预防措施，预防措施限制字数为10字以下短语。',
    'disease_acompany_disease':'该关系类型描述的是疾病间的关系，展现了疾病所可能伴随的并发症。主体是疾病，客体是另一种疾病。',
    'disease_has_cause':'该关系描述的是疾病和病因的关系，展现了导致患病的可能原因。主体是疾病，客体是病因，病因是各种名词或名词短语。',
    'disease_has_spreads_way':'该关系描述的是疾病和传播方法的关系，展现了疾病通过哪些途径传播。主体是疾病，客体是传播方法，治疗方法限制字数为10字以下短语。',
    }
...
def generate_output(texts):
    all_results = []
    texts = [texts[i:i + 2048] for i in range(0, len(texts), 2048)]
    for text in texts:
        # 构造输入并进行预测
        sintructs = get_instruction(language, task, schema, text)  # 适当修改以适应您的schema和任务
...
def json_load():
    ...
    with open('data/raw_data/instructions.json', 'r') as fh:
        data = json.load(fh)
...
def process_json():
    for item in json_load():
        ...
        output_file_path = os.path.join(output_folder, f'result_instruction.json')
